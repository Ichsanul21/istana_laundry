<x-admin-layout>
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
        <h2 class="text-xl font-semibold text-gray-800 dark:text-white/90">FAQ</h2>
        <nav>
            <ol class="flex items-center gap-1.5">
                ...
                <li class="text-theme-sm text-gray-800 dark:text-white/90">FAQ</li>
            </ol>
        </nav>
    </div>
    ...
    <div class="flex items-center justify-between mb-4">
        <p class="text-theme-sm text-gray-500 dark:text-gray-400">Total {{ $faqs->count() }} FAQ</p>
        <a href="{{ route('admin.faqs.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 text-theme-sm font-medium text-white bg-brand-500 rounded-lg hover:bg-brand-600 transition-colors">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.5v15m7.5-7.5h-15"/></svg>
            Tambah FAQ
        </a>
    </div>

    <x-ui.table :headers="['No', 'Pertanyaan', 'Kategori', 'Status', 'Aksi']">
        @forelse($faqs as $faq)
            <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02]">
                <td class="px-5 py-3 text-theme-sm text-gray-500 dark:text-gray-400">{{ $loop->iteration }}</td>
                <td class="px-5 py-3">
                    <span class="text-theme-sm font-medium text-gray-800 dark:text-white/90">{{ Str::limit($faq->question, 80) }}</span>
                </td>
                <td class="px-5 py-3">
                    @if($faq->category)
                        <x-ui.badge color="info" size="sm">{{ $faq->category }}</x-ui.badge>
                    @else
                        <span class="text-theme-sm text-gray-400">-</span>
                    @endif
                </td>
                <td class="px-5 py-3">
                    <x-ui.badge :color="$faq->is_active ? 'success' : 'error'" size="sm">{{ $faq->is_active ? 'Aktif' : 'Nonaktif' }}</x-ui.badge>
                </td>
                <td class="px-5 py-3">
                    <div class="flex items-center gap-2">
                        <a href="{{ route('admin.faqs.edit', $faq) }}" class="text-gray-500 hover:text-brand-500 dark:text-gray-400 dark:hover:text-brand-400 transition-colors" title="Edit">
        ...
        @empty
            <tr><td colspan="5" class="px-5 py-12 text-center text-theme-sm text-gray-500 dark:text-gray-400">Belum ada FAQ.</td></tr>
        @endforelse
    </x-ui.table>
</x-admin-layout>
